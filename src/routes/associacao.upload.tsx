...
function UploadPlanilha() {
  ...
  const dadosSimulados: RegistroPlanilha[] = [
    { servidor: "João da Silva", matricula: "12345678", beneficiario: "João da Silva", cpf: "123.***.***-00", vinculo: "Titular", valor: 1200, status: "válido" },
    { servidor: "João da Silva", matricula: "12345678", beneficiario: "Ana da Silva", cpf: "234.***.***-11", vinculo: "Cônjuge", valor: 890, status: "válido" },
    { servidor: "Maria Oliveira", matricula: "87654321", beneficiario: "Maria Oliveira", cpf: "345.***.***-22", vinculo: "Titular", valor: 1800, status: "válido" },
    { servidor: "Maria Oliveira", matricula: "87654321", beneficiario: "José Oliveira", cpf: "", vinculo: "Pai", valor: 1100, status: "não_elegível", motivo: "Vínculo não elegível para o Pró-Saúde" },
    { servidor: "Ricardo Mendes", matricula: "99887766", beneficiario: "Ricardo Mendes", cpf: "456.***.***-33", vinculo: "Titular", valor: 950, status: "atenção", motivo: "Número do processo SEI ausente" },
    { servidor: "Ricardo Mendes", matricula: "99887766", beneficiario: "Beatriz Mendes", cpf: "567.***.***-44", vinculo: "Mãe", valor: 950, status: "não_elegível", motivo: "Vínculo não elegível para o Pró-Saúde" },
  ];
              ...
              <div className="p-6 space-y-6">
                ...
                <div className="bg-amber-50 border border-amber-100 rounded-xl p-4 flex gap-3">
                  ...
                  <div className="text-xs text-amber-800 leading-relaxed">
                    <p className="font-bold mb-1">Atenção sobre beneficiários:</p>
                    <p>
                      Pessoas que não se enquadram nas regras do Pró-Saúde, como pais, mães, irmãos ou outros vínculos não previstos, não devem ser enviadas como beneficiários do programa. Caso constem na planilha da associação, elas deverão ser identificadas para conferência e não consideradas no cálculo do ressarcimento.
                    </p>
                  </div>
                </div>
          ...
          <div className="space-y-6">
            <div className="bg-slate-900 rounded-2xl p-6 text-white space-y-4">
              ...
              <p className="text-xs text-slate-400 leading-relaxed">
                Baixe o modelo padronizado para garantir que o envio seja processado corretamente. Esta é uma versão preliminar.
              </p>
              ...
              <div className="pt-4 border-t border-slate-800">
                <p className="text-[10px] uppercase font-bold text-slate-500 tracking-wider mb-3">Colunas do Modelo</p>
                <div className="grid grid-cols-1 gap-1.5 text-[10px] text-slate-300">
                  {["Competência", "Matrícula do Servidor", "CPF do Beneficiário", "Nome do Beneficiário", "Parentesco/Vínculo", "Valor Mensal Individual", "Data de Pagamento"].map(col => (
                    <div key={col} className="flex items-center gap-2">
                      <div className="h-1 w-1 bg-primary rounded-full" />
                      {col}
                    </div>
                  ))}
                  <p className="text-slate-500 italic mt-1">+ outras 12 colunas</p>
                </div>
              </div>
            </div>
          </div>
      ...
      {step === "conferencia" && (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <CardResumo label="Registros Válidos" value={validos} icon={CheckCircle2} color="green" />
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            ...
            <div className="p-6 bg-slate-50 border-t border-slate-100 flex justify-between items-center">
              ...
              <button 
                ...
                className="bg-primary text-white px-8 py-3 rounded-lg font-bold flex items-center gap-2 hover:bg-primary-dark shadow-lg shadow-primary/20 transition"
              >
