...
**Elegibilidade por tipo de assistência (`getElegibilidade`, em `comprovante-status.ts`) — snippet histórico, ver seção 3.26 para a versão atual:**
```ts
...
```
**Superada pela Etapa 4 (seção 3.21) e, em seguida, pela avaliação por item da seção 3.26** — assinatura e retorno mudaram (`beneficiarioId` → `beneficiario: BeneficiarioPagamento`; `{ elegivel, tipoAssistencia? }` → `{ elegivel, decomposicao }`), mas a ideia central descrita abaixo (função pura, ausência do botão em vez de `disabled`, banner compartilhado pelos 3 perfis) permanece válida.
...
### 3.21 Taxonomia de campos por tipo documental + situações não reembolsáveis (Etapa 4)

**Contexto:** Etapa 4 do alinhamento de stakeholder (ver seção 13, decisão 33). Duas mudanças independentes pedidas: (1) `CampoExtraido` deixa de ter uma lista única de campos possíveis para todo documento — cada tipo documental (Boleto, Recibo, Demonstrativo, Fatura Técnica, Comprovante de Pagamento) passa a extrair uma combinação própria e literal, definida pelo stakeholder; (2) Banco e Tipo de Assistência deixam de ser campos do formulário — a IA passa a **detectar automaticamente** situações não reembolsáveis (odontológico, multa, taxa administrativa, juros) e exibi-las como alerta, não como campo editável.

**Nova taxonomia** (`camposPorTipoDocumento`, `ocr-mock.ts`) — lista literal do stakeholder:
- **Comprovante de Pagamento** (fonte principal): `pagador`, `valor`, `dataPagamento`, `vencimento`, `operadora`.
- **Boleto**: `nome`, `operadora`, `competencia`, `vencimento`, `valor`.
- **Recibo / Demonstrativo**: `nome`, `competencia`, `vencimento`, `valor` — **sem** `operadora` (simplificação assumida, decidida pelo stakeholder na revisão do plano): um envio só-recibo/só-demonstrativo não terá operadora extraída, fica "não identificado". Isso também significa que a checagem de operadora divergente do cadastro (Etapa 6) simplesmente não se aplica a esse caso.
- **Fatura Técnica**: `nome`, `operadora`, `competencia`, `vencimento` (sem `valor` — os valores individualizados por beneficiário já são cobertos pela arquitetura existente de `gruposExtraidos`; Empresarial sempre exige Fatura Técnica **e** Comprovante juntos, então o valor vem do Comprovante).
- `CampoExtraido['chave']` perdeu `banco` e `tipoAssistencia`; ganhou `vencimento`. `dataPagamento` e `pagador` ficaram **exclusivos** do Comprovante de Pagamento — é o único documento que realmente comprova quando e por quem o pagamento foi feito; Boleto/Recibo/Demonstrativo trazem `vencimento` (data impressa no documento) no lugar disso.

**Regra do Pagador — confirmada, sem mudança de comportamento:** a única comparação de "quem pagou" continua sendo `pagador` (agora exclusivo de Comprovante de Pagamento) contra o titular do grupo familiar. O campo `nome` de um Boleto nunca é comparado com o titular — é só informativo. Como `nome` e `pagador` já eram chaves distintas antes da Etapa 4, nenhuma mudança de código foi necessária além de confirmar que a nova taxonomia não introduziu uma comparação nova por engano.

**Nova validação cruzada Boleto × Comprovante** (`getDivergenciaBoletoComprovante`, `comprovante-status.ts`): quando um beneficiário tem Boleto **e** Comprovante de Pagamento cobrindo-o (mesmo que em arquivos diferentes), compara o `valor` bruto de cada um **recomputado sob demanda** a partir dos arquivos persistidos — `gerarCamposExtraidos` é determinístico, então não é preciso guardar um campo novo; a consolidação (`mesclarCamposDeArquivos`) já descarta o valor do arquivo que "perde", então comparar os 2 lados exige reprocessar cada arquivo isoladamente por tipo. Se diferentes, badge "Valor diverge do boleto anexado" em `CamposExtraidosForm`, mesmo padrão visual do badge "Divergente" já existente. **Testado manualmente:** Boleto normal + Comprovante com nome "divergente" (valor +100) → badge aparece corretamente junto ao campo Valor, tanto na conferência do Servidor quanto na tela do Analista/Gerência.

**Situações não reembolsáveis generalizadas** (`detectarSituacaoNaoReembolsavel`, `ocr-mock.ts`, substitui o antigo `arquivoEhOdontologico`):
```ts
type SituacaoNaoReembolsavel = 'odontologico' | 'multa' | 'taxa_administrativa' | 'juros';
```
- Não é mais um `CampoExtraido` (não é campo de formulário) — é uma propriedade **calculada** por `getSituacaoNaoReembolsavel(comprovante, beneficiarioId)` (`comprovante-status.ts`), que varre os arquivos que cobrem aquele beneficiário (mesma função `beneficiariosCobertosPeloDocumento` do checklist da Etapa 3) e testa o nome de cada um contra as 4 palavras-chave. `getElegibilidade` passou a devolver `{ elegivel, situacao? }` em vez de `{ elegivel, tipoAssistencia? }`. **Superado pela seção 3.26** (ajuste pontual pós-Etapa 7): `getSituacaoNaoReembolsavel` foi removida (a detecção passou a ser por `getItensFinanceiros`/`getDecomposicaoValor`, item a item, não mais 1 situação por documento inteiro); `getElegibilidade` passou a devolver `{ elegivel, decomposicao }`.
- **Banner único, implementado 1 vez em `CamposExtraidosForm.tsx`** (novo prop `situacaoNaoReembolsavel`, **substituída na seção 3.26 por `decomposicaoValor`**), em vez do banner que existia só em `admin.comprovantes.tsx` — agora aparece igualmente para Servidor (`ConferenciaBeneficiarios.tsx`, `ServidorComprovanteDetail.tsx`), Analista/Gerência (`admin.comprovantes.tsx`) e na edição inline do Resumo (`ConsolidadoCompetencia.tsx`). O banner antigo, mais detalhado e só-admin, foi removido de `admin.comprovantes.tsx` para não duplicar a mensagem.
- Bloqueio de "Aprovar"/"Aprovar com ressalva" continua exatamente igual ao já implementado para odontológico (ausência do botão no DOM, não um `disabled`) — só que agora cobre as 4 situações **por item, não pelo documento inteiro quando há mensalidade reembolsável no mesmo arquivo (seção 3.26)**.
- **`ConferenciaBeneficiarios.tsx` (servidor, antes de persistir) monta um "pseudo-Comprovante"** (`{ arquivos, beneficiarioIds, competencia }`, ganhou a prop `competencia`) para poder chamar `getElegibilidade`/`getDivergenciaBoletoComprovante` mesmo sem um `Comprovante` real ainda existir — as duas funções aceitam `Pick<Comprovante, ...>` propositalmente para isso.
- Documentado explicitamente (já valia para odontológico, agora vale para as 4 situações): no protótipo a detecção continua por nome de arquivo; no sistema real seria por conteúdo do documento (OCR/IA real).

**Seed (`mock-data.ts`)** atualizado: os 5 comprovantes de exemplo trocaram `banco` por `vencimento` e os 2 baseados em Recibo/Demonstrativo perderam o campo `operadora`, para refletir a nova taxonomia desde o primeiro carregamento do protótipo.

**Testado manualmente no navegador:** taxonomia por tipo (Recibo mostrando só Nome/CPF/Competência/Valor/Vencimento, sem Operadora/Banco); banner "Não elegível — Taxa administrativa"/"Não elegível — Multa" visível tanto na conferência do Servidor (antes do envio) quanto na fila do Analista/Gerência (depois); badge "Valor diverge do boleto anexado" nos dois contextos; ausência de "Aprovar"/"Aprovar com ressalva" no comprovante não elegível.

---

### 3.22 Justificativas obrigatórias — retroativo e divergência de valor (Etapa 5)

**Contexto:** Etapa 5 do alinhamento de stakeholder (ver seção 13, decisão 34). Duas justificativas que já eram exigidas do Servidor, mas só como "não vazias", passam a exigir um mínimo de conteúdo real — e a divergência de valor ganha uma justificativa nova, escrita pelo próprio Servidor no momento do envio (distinta da justificativa que o Analista/Gerência já escrevia ao aprovar com ressalva).

**Helper novo** (`src/lib/validation-pagamento.ts`):
```ts
function temPeloMenosNPalavras(texto: string, n = 3): boolean {
  return texto.trim().split(/\s+/).filter(Boolean).length >= n;
}
```

**Retroativo:** `podeAvancarSelecao` (`servidor.pagamentos.enviar.tsx`) passou a chamar `temPeloMenosNPalavras(justificativaAtraso)` em vez de só checar `.length > 0`. A textarea mostra "Escreva pelo menos 3 palavras." em vermelho enquanto o texto digitado não atinge o mínimo (só depois de já haver algum texto — não antes de o Servidor começar a digitar).

**Divergência de valor:** em `ConferenciaBeneficiarios.tsx`, quando `valorDivergeDoCadastro(campos, beneficiario.valorCadastrado)` (helper puro extraído de `getDivergencia`, `comprovante-status.ts`) indica divergência, uma textarea "Justificativa da divergência de valor" aparece logo abaixo do formulário de campos daquele beneficiário — mesma mensagem de erro/mesmo padrão visual da justificativa de retroativo. O botão "Confirmar" daquele beneficiário fica desabilitado até a justificativa atingir 3 palavras, além da checagem de campos não identificados já existente. A auto-confirmação por alta confiança (efeito que confirma sozinho beneficiários com todos os campos em confiança "alta") também passou a **pular** quem tem valor divergente — não faria sentido confirmar automaticamente um caso que exige justificativa manual.
...
### 3.23 Operadora divergente do cadastro → oferta de requerimento de mudança de plano (Etapa 6)

**Contexto:** Etapa 6 do alinhamento de stakeholder (ver seção 13, decisão 35). Diferente da divergência de **valor** (Etapa 5, bloqueante até justificativa), a divergência de **operadora** é resolvida com uma escolha explícita do Servidor entre 2 caminhos, sem exigir texto livre.

**Detecção** (`operadoraDivergeDoCadastro`, `comprovante-status.ts`, mesmo padrão de `valorDivergeDoCadastro`): compara o campo `operadora` já consolidado em `grupo.campos` com `beneficiario.operadora`. Só considera divergência quando o campo foi de fato extraído — Recibo/Demonstrativo nunca extraem `operadora` (taxonomia da Etapa 4), então nunca acionam este alerta para esses tipos documentais, mesma consequência já assumida e documentada na Etapa 4.

**Aviso em `ConferenciaBeneficiarios.tsx`:** quando divergente, aparece por beneficiário (mesmo bloco de alertas onde já aparecia a justificativa de valor) o texto *"A operadora identificada ({X}) é diferente do seu cadastro ({Y})."*, com 2 ações:
- **"Abrir requerimento de mudança de plano"** — `Link` para `/servidor/requerimento/novo-plano`, aberto **em nova aba** (`target="_blank"`). Decisão explícita: o wizard de requerimento é um fluxo próprio e extenso (6 passos, estado independente), e o wizard de pagamento ainda não persistiu nada até a confirmação final — navegar na mesma aba perderia todo o progresso do envio em andamento. Abrir em nova aba preserva os dois fluxos.
- **"Continuar mesmo assim"** — marca aquele beneficiário como confirmado apesar da divergência; o botão vira "Continuar mesmo assim ✓" (mesmo padrão visual de estado confirmado usado em outros lugares do protótipo).

O botão "Confirmar" daquele beneficiário fica desabilitado até "Continuar mesmo assim" ser clicado — abrir o requerimento sozinho não desbloqueia (é uma ação informativa/paralela, não uma resolução). A auto-confirmação por alta confiança também passou a pular beneficiários com operadora divergente, pela mesma razão já aplicada à divergência de valor (Etapa 5): a decisão precisa ser explícita do Servidor.
...
### 3.26 Elegibilidade por item financeiro — exclusão granular, não mais por documento inteiro (ajuste pontual, pós-Etapa 7)

**Contexto:** ajuste pontual pedido pelo usuário depois do commit da Etapa 7 (ver seção 13, decisão 38), a partir de um exemplo real (boleto Hapvida com "Mensalidade Plano Saúde" e "Valor de Odontologia" como linhas separadas do mesmo documento). Até aqui (seção 3.21), uma situação não reembolsável era uma propriedade **do documento inteiro**: se o nome do arquivo indicasse odontológico/multa/taxa administrativa/juros, **todo** o beneficiário ficava não elegível, mesmo quando o mesmo documento também comprovava a mensalidade normal do plano — que deveria continuar sendo reembolsável. O usuário rejeitou explicitamente a alternativa mais simples (um booleano manual `documentoMisto`) e pediu que a elegibilidade fosse avaliada **por item/valor identificado no documento**, não pelo documento como um todo.

**Novo tipo `ItemFinanceiro`** (`mock-data.ts`), com valores mock fixos e ilustrativos para os itens não reembolsáveis (decisão explícita do usuário — **não** usar percentual do valor cadastrado, para não introduzir uma regra de negócio inexistente só para o mock):
```ts
interface ItemFinanceiro {
  descricao: string;
  valor: number;
  reembolsavel: boolean;
  situacaoNaoReembolsavel?: SituacaoNaoReembolsavel; // presente só quando reembolsavel === false
}

const valorMockItemNaoReembolsavel: Record<SituacaoNaoReembolsavel, number> = {
  odontologico: 46.33,
...
**Geração mock por arquivo** (`gerarItensFinanceiros(beneficiario, nomeArquivo)`, `ocr-mock.ts`) — convenção de nome de arquivo **preservada e estendida**, para não quebrar os cenários de teste que já existiam desde a Etapa 4:
- Nenhuma situação detectada → 1 item só, `{ descricao: "Mensalidade Plano de Saúde", valor: valorCadastrado (±100 se `divergente`), reembolsavel: true }`.
- Situação(ões) detectada(s) **sem** `"misto"` no nome do arquivo (ex: `..._odontologico.jpg`) → só o(s) item(ns) não reembolsável(is), **sem** item de mensalidade — mesmo comportamento de 100%-bloqueio já testado desde a Etapa 4, bit a bit preservado.
- Situação(ões) detectada(s) **com** `"misto"` no nome do arquivo (ex: `..._odontologico_misto.jpg`, `..._multa_misto.jpg`) → item de mensalidade (reembolsável) **+** o(s) item(ns) não reembolsável(is) — o cenário novo pedido pelo usuário.
- `detectarSituacaoNaoReembolsavel` (singular, 1º match) foi substituído por `detectarSituacoesNaoReembolsaveis` (plural) — um mesmo arquivo pode combinar mais de uma situação (ex: `..._odontologico_juros_misto.jpg`), o que não era possível detectar antes.

**Decomposição e elegibilidade recalculadas sob demanda** (`comprovante-status.ts`, mesmo padrão "recompute a partir de `arquivos`, nunca persistir" já usado por `getDivergenciaBoletoComprovante` — necessário porque `ConferenciaBeneficiarios` precisa da mesma informação **antes** de existir um `Comprovante` persistido, via o mesmo "pseudo-Comprovante" já usado desde a Etapa 4):
```ts
function getItensFinanceiros(comprovante, beneficiario): ItemFinanceiro[]

interface DecomposicaoValor {
  itens: ItemFinanceiro[];
  valorTotal: number;          // soma de todos os itens (reembolsáveis + não)
  valorElegivel: number;       // soma só dos itens reembolsáveis
  valorNaoReembolsavel: number; // valorTotal - valorElegivel
}
function getDecomposicaoValor(itens: ItemFinanceiro[]): DecomposicaoValor

function getElegibilidade(comprovante, beneficiario): { elegivel: boolean; decomposicao: DecomposicaoValor }
// elegivel = !temItemNaoReembolsavel || decomposicao.valorElegivel > 0
```
`getElegibilidade` mudou de assinatura (recebia `beneficiarioId: string`, passou a receber o `BeneficiarioPagamento` inteiro — necessário para `gerarItensFinanceiros` calcular o valor da mensalidade) e de retorno (`{ elegivel, situacao? }` → `{ elegivel, decomposicao }`). Todo consumidor (`ConferenciaBeneficiarios.tsx`, `admin.comprovantes.tsx`, `ServidorComprovanteDetail.tsx`, `ConsolidadoCompetencia.tsx`) foi atualizado com o mesmo padrão defensivo já usado para `getDivergenciaBoletoComprovante`: `beneficiario ? getElegibilidade(x, beneficiario) : { decomposicao: <vazia> }`.

**Preservação explícita da semântica do campo `valor` (exigência direta do usuário, ao aprovar o modelo proposto):** o campo `valor` de `CampoExtraido` **continua significando o valor bruto/total do documento** — não foi silenciosamente redefinido para "valor elegível". A fórmula mudou (`gerarCamposExtraidos`, `ocr-mock.ts`): antes ecoava `valorCadastrado` (±100 se `divergente`) direto; agora é a soma de todos os `ItemFinanceiro` gerados (`valorTotal`) — resultado **idêntico, byte a byte**, para os casos normal/misto (mensalidade sozinha ou mensalidade + item), e **corrigido** (deixa de ecoar o cadastro) só no caso 100%-não-reembolsável, onde antes o campo Valor mostrava enganosamente o valor do plano mesmo sem nenhum item de mensalidade no documento. `valorElegivel`, `valorNaoReembolsavel` e `valorTotal` são consultáveis separadamente via `DecomposicaoValor` — nenhum deles substitui ou esconde o outro.

**Divergência cadastral passa a comparar `valorElegivel`, não o bruto** (`valorDivergeDoCadastro`, `getDivergencia`) — exigência explícita do usuário: "a divergência cadastral deve comparar `valorElegivel` com `valorCadastrado`". Consequência **assumida e testada**, não uma regressão: num documento 100%-não-reembolsável, `valorElegivel = 0` diverge naturalmente de `valorCadastrado`, o que também aciona a exigência de justificativa da Etapa 5 (`ConferenciaBeneficiarios`) — comportamento correto (o Servidor está, de fato, tentando enviar um documento sem nenhum valor reembolsável), não um bug. **A divergência documental (Boleto × Comprovante, `getDivergenciaBoletoComprovante`, seção 3.21) não foi tocada** — continua comparando valores brutos por tipo documental, agora automaticamente mais precisa (porque `gerarCamposExtraidos` já reflete a decomposição por item), sem nenhuma mudança de código nela.

**`CamposExtraidosForm.tsx`:** prop `situacaoNaoReembolsavel?: SituacaoNaoReembolsavel` substituída por `decomposicaoValor?: DecomposicaoValor`. O banner ganhou **2 estados visuais**, em vez de 1 só:
- **Bloqueante (vermelho)**, quando `valorElegivel <= 0`: mesmo texto de sempre — *"Não elegível — {situação(ões)}. Situação não reembolsável pelo Pró-Saúde; aprovação automática ou com ressalva fica bloqueada até revisão da GERDAB."*
- **Informativo (amarelo)**, quando há item(ns) não reembolsável(is) mas `valorElegivel > 0` (cenário misto, novo): *"Itens não reembolsáveis identificados neste documento — desconsiderados do valor elegível."* — aprovação **permanece disponível**.

Em ambos os casos, o banner lista os itens não reembolsáveis (descrição + valor + situação) e uma linha de resumo "Valor total: ... · Valor elegível: ... · Valor não reembolsável: ...". Badge do campo `valor` ("Valor difere do cadastro") passou a comparar `decomposicaoValor.valorElegivel` em vez de `parseFloat(campo.valor)`.

**Sem UI nova, confirmado pelo usuário:** nenhum modal ou tela adicional foi criada para o cenário misto — o banner informativo em `CamposExtraidosForm` (compartilhado por Servidor, Analista e Gerência, mesmo componente de sempre) é suficiente; os botões "Aprovar"/"Aprovar com ressalva" simplesmente continuam visíveis quando `valorElegivel > 0`.

**`ConsolidadoCompetencia.tsx` (Resumo da Competência):** cada linha de beneficiário mostra `valorElegivel` como valor principal do reembolso (era o único valor mostrado antes); quando há valor não reembolsável (`totalNaoReembolsavel > 0`), uma linha secundária "Bruto {valorTotal} · Não reembolsável {totalNaoReembolsavel}" aparece logo abaixo, discreta — decomposição sempre acessível, sem esconder informação, mas sem competir visualmente com o valor que efetivamente importa para o total do grupo familiar.

**Testado manualmente no navegador:** envio de Recibo `recibo_odontologico_misto_carlos.jpg` para Carlos (mensalidade R$ 420,00 + odontológico R$ 46,33) → banner amarelo informativo, "Aprovar"/"Aprovar com ressalva" visíveis na conferência do Servidor (sem bloqueio) e na fila da Gerência; decomposição exibida corretamente em ambos os lados ("Valor total: R$ 466,33 · Valor elegível: R$ 420,00 · Valor não reembolsável: R$ 46,33"); clique em "Aprovar" pela Gerência aprova **diretamente**, sem abrir `DivergenciaAprovacaoModal` (`valorElegivel` bate exatamente com o cadastro de Carlos); no Resumo da Competência, a linha de Carlos mostra "R$ 1.360,00" como valor principal com "Bruto R$ 1.406,33 · Não reembolsável R$ 46,33" como sub-linha (soma de todos os envios da competência, incluindo o misto); cenário 100%-não-reembolsável (só item odontológico, sem `misto`) re-testado e confirmado **inalterado**: banner vermelho bloqueante, botões de aprovação ausentes do DOM para Servidor/Analista/Gerência.
...
- **Aprovar** — chama `aprovar(comprovante, beneficiarioId)`. **Não renderizado** quando `!getElegibilidade(cur, beneficiario).elegivel` — elegibilidade avaliada por item financeiro, não pelo documento inteiro (ver seção 3.26; superou a seção 3.18).
...
**(Corrigido — ajuste pontual, ver seção 13)** `retroativo_aguardando_analista` e `retroativo_aguardando_gerencia` são a **mesma pendência** que `retroativo_aguardando_aprovacao`, só com o nome de status anterior à Etapa 1 — `statusComAcaoDisponivel` os inclui, então um registro nesses status (de antes da renomeação) é acionável por qualquer um dos dois papéis, exatamente como qualquer outro retroativo pendente. O rótulo exibido (`statusComprovanteLabels`) também foi unificado para "Retroativo — Aguardando Aprovação", sem sufixo "(legado)". Só `retroativo_devolvido` continua **somente leitura** — representa uma devolução já decidida sob a antiga 2ª alçada, não uma pendência equivalente à aprovação única.
...
Antes da Etapa 1 (ver plano de alinhamento de stakeholder, seção 13, decisão 30), retroativo passava por `retroativo_aguardando_analista → retroativo_aguardando_gerencia → retroativo_aprovado`, com um botão exclusivo "Devolver ao Analista" (status `retroativo_devolvido`) disponível só para a Gerência. Essa mecânica foi **removida do fluxo ativo** a pedido do stakeholder ("não deve mais existir a necessidade de segunda alçada, pode ser o Analista ou a Gerência"). Os 3 status legados continuam no union `StatusComprovante` e em `statusPorTab.retroativos` para exibir registros antigos já persistidos em `localStorage` de sessões de teste anteriores.

**(Corrigido — ajuste pontual, ver seção 13)** Nem todo status legado é "somente histórico": `retroativo_aguardando_analista`/`retroativo_aguardando_gerencia` representam uma pendência **ainda não decidida** — a mesma coisa que `retroativo_aguardando_aprovacao` hoje, só com o nome antigo — então continuam **acionáveis** por qualquer um dos dois papéis, sem exigir uma 2ª alçada. Um registro assim resolve (aprova/recusa/etc.) exatamente como qualquer outro retroativo pendente, e vira `retroativo_aprovado`/`retroativo_recusado` normalmente ao ser decidido — a mensagem "Registro legado (2ª alçada) — somente consulta" foi removida por não refletir mais o comportamento. Já `retroativo_devolvido` representa uma decisão **já tomada** sob a antiga 2ª alçada (a Gerência já devolveu ao Analista) — esse sim continua **somente leitura**, sem nenhum botão de ação, exatamente como antes.
...
6c. **(Nova — Etapa 6, ver seção 3.23)** Quando a `operadora` extraída de um documento diverge da operadora cadastrada do beneficiário, o Servidor precisa escolher explicitamente entre abrir um requerimento de mudança de plano (nova aba, sem bloquear o envio atual) ou "Continuar mesmo assim" — só essa segunda opção libera a confirmação daquele beneficiário. Diferente da divergência de valor (regra 26a), não exige texto livre, só uma decisão binária.
...
14a. **(Atualizado — Etapa 5, ver seção 3.22)** A justificativa do atraso precisa ter **pelo menos 3 palavras** (`temPeloMenosNPalavras`, `src/lib/validation-pagamento.ts`), não só ser não-vazia.
...
### Situações não reembolsáveis (odontológico, multa, taxa administrativa, juros) — implementado (seções 3.18, 3.21)
31d. **(Atualizado — Etapa 4, ver seção 13, decisão 33)** `tipoAssistencia` deixou de ser um `CampoExtraido` editável — Banco e Tipo de Assistência foram removidos do formulário. A detecção passou a ser uma propriedade calculada: `SituacaoNaoReembolsavel = 'odontologico' | 'multa' | 'taxa_administrativa' | 'juros'`, obtida por `getSituacaoNaoReembolsavel(comprovante, beneficiarioId)` (`comprovante-status.ts`), que varre os arquivos que cobrem aquele beneficiário e testa o nome de cada um via `detectarSituacaoNaoReembolsavel()` (`ocr-mock.ts`) contra as 4 palavras-chave. Exibida como banner único em `CamposExtraidosForm` (prop `situacaoNaoReembolsavel`), visível para Servidor, Analista e Gerência — não só admin como antes.
31e. **Bloqueio de aprovação implementado, mesma mecânica de antes:** `getElegibilidade(comprovante, beneficiarioId)` (`comprovante-status.ts`) retorna `{ elegivel: false, situacao }` quando alguma das 4 situações é detectada. Em `admin.comprovantes.tsx`, quando não elegível: os botões "Aprovar" e "Aprovar com ressalva" **não são renderizados** para aquele beneficiário (nem pelo Analista, nem pela Gerência) — só "Solicitar correção", "Recusar" e "Solicitar documento complementar" continuam disponíveis. Não há um segundo bloqueio dentro de `aprovar()` (a função nunca é chamada, pois o botão não existe) — o controle é 100% via ausência do botão na UI.
...
interface CampoExtraido {
  chave:
    | 'nome' | 'cpf' | 'operadora' | 'competencia' | 'vencimento'
    | 'valor' | 'dataPagamento' | 'pagador';
  valor: string;
  origem: 'ocr' | 'manual';
  confianca: 'alta' | 'media' | 'nenhuma';
  arquivoOrigem?: string; // nome do arquivo (dentre os anexados ao envio) que originou este campo
}

type SituacaoNaoReembolsavel = 'odontologico' | 'multa' | 'taxa_administrativa' | 'juros';
```
`pagador` foi adicionado antes (regra do usuário — pagamento deve ser feito pelo titular). `arquivoOrigem` foi adicionado na rodada de multi-arquivo (seção 3.17) — sustenta a rastreabilidade de qual arquivo originou cada campo consolidado. `cpf` existe no tipo e nos dados seed, mas **não é gerado** por `gerarCamposExtraidos()` (assimetria conhecida, seção 12). **(Atualizado — Etapa 4, seção 3.21)** `banco` e `tipoAssistencia` foram removidos (não são mais campos de formulário); `vencimento` foi adicionado — cada tipo documental agora extrai uma combinação própria de chaves (`camposPorTipoDocumento`, `ocr-mock.ts`), não mais uma lista genérica igual para todos. `TipoAssistencia`/`tipoAssistenciaLabels` foram substituídos por `SituacaoNaoReembolsavel`/`situacaoNaoReembolsavelLabels` — não é mais um valor de campo, é uma propriedade calculada (ver seção 6, "Situações não reembolsáveis").

**(Atualizado — ajuste pontual pós-Etapa 7, seção 3.26) O campo `valor` continua sendo o valor bruto/total do documento** — não foi redefinido para "valor elegível". A fórmula que o gera mudou (agora é a soma de todos os `ItemFinanceiro` do documento, ver abaixo), mas o significado do campo, para quem já o consumia (badge de divergência **documental** Boleto × Comprovante, seção 3.21), não mudou.

### `ItemFinanceiro` e `DecomposicaoValor` (novo — ajuste pontual pós-Etapa 7, seção 3.26)
```ts
interface ItemFinanceiro {
  descricao: string;
  valor: number;
...
const valorMockItemNaoReembolsavel: Record<SituacaoNaoReembolsavel, number> = {
  ...
  multa: 20.00,
  juros: 10.00,
}; // valores fixos ilustrativos do mock — não é percentual do valor cadastrado, decisão explícita do usuário
```
`DecomposicaoValor` (`{ itens, valorTotal, valorElegivel, valorNaoReembolsavel }`) é calculada por `getDecomposicaoValor()` (`comprovante-status.ts`) a partir da lista de `ItemFinanceiro` — **nenhum dos dois tipos é persistido em `Comprovante`**, ambos são recomputados sob demanda a partir de `arquivos` (mesmo padrão de `getDivergenciaBoletoComprovante`), para funcionar tanto antes quanto depois da persistência. A elegibilidade do beneficiário deixou de ser avaliada pelo documento inteiro e passou a ser avaliada **por item**: um mesmo arquivo pode ter um item reembolsável (mensalidade) e um item não reembolsável (ex: odontológico) ao mesmo tempo — `elegivel = !temItemNaoReembolsavel || valorElegivel > 0`. Ver seção 3.26 para a regra completa.
...
interface Comprovante {
  ...
  justificativasDivergencia?: { beneficiarioId: string; texto: string }[]; // ATUALIZADO — Etapa 5 (seção 3.22): generaliza o antigo `justificativaDivergencia?: string` (morto) para suportar multi-beneficiário; escrita pelo Servidor no envio, exigida (≥3 palavras) quando o valor diverge do cadastro
  operadoraDivergenteCadastro?: boolean;   // NOVO — Etapa 6 (seção 3.23): true quando o Servidor escolheu "Continuar mesmo assim" apesar da operadora identificada divergir do cadastro de algum beneficiário
...
### `CamposExtraidosForm`
- **Finalidade:** exibir e (opcionalmente) editar o conjunto de `CampoExtraido[]` de 1 beneficiário, com indicadores de confiança, origem do arquivo, divergência e alertas de elegibilidade.
- **Props:** `titulo?: string`, `campos: CampoExtraido[]`, `onChange?: (campos) => void`, `valorCadastrado?: number` (ativa divergência de `valor`, comparando `valorElegivel` — não o bruto, ver seção 3.26), `nomeTitular?: string` (ativa divergência de `pagador`), `decomposicaoValor?: DecomposicaoValor` (**atualizado — ajuste pontual pós-Etapa 7, seção 3.26**, substitui o antigo `situacaoNaoReembolsavel?: SituacaoNaoReembolsavel` da Etapa 4; banner de alerta + decomposição), `divergenciaBoletoComprovante?: boolean` (**novo — Etapa 4**, badge no campo `valor`), `readOnly?: boolean` (default `false`).
- **Comportamento:** banner no topo do form quando `decomposicaoValor` tem algum item não reembolsável — **vermelho/bloqueante** ("Não elegível — {situação(ões)}. ... bloqueada até revisão da GERDAB") quando `valorElegivel <= 0`, **amarelo/informativo** ("Itens não reembolsáveis identificados... desconsiderados do valor elegível") quando `valorElegivel > 0` (cenário misto); em ambos os casos lista os itens (descrição + valor + situação) e uma linha "Valor total: ... · Valor elegível: ... · Valor não reembolsável: ...". Por campo, mostra label + input; badges condicionais "Não identificado" (campo vazio), "Valor difere do cadastro" (só no campo `valor`, compara `valorElegivel`), "Divergente" (só no campo `pagador`), "Valor diverge do boleto anexado" (**Etapa 4**, só no campo `valor` quando `divergenciaBoletoComprovante`, compara valores brutos — independente da divergência cadastral), "Preenchido manualmente" (`origem === 'manual'`) ou indicador de confiança (ícone + texto); linha adicional "Origem: {arquivoOrigem}" quando o campo tem essa informação. Editar sempre seta `origem: 'manual'`.
- **Usado em:** `ConferenciaBeneficiarios.tsx` (editável; **Etapa 4** — ganhou a prop `competencia` para poder recalcular elegibilidade/divergência a partir de um "pseudo-Comprovante" antes de persistir), `ServidorComprovanteDetail.tsx` (misto: editável durante reenvio, `readOnly` nos demais estados; no reenvio a situação é calculada direto do nome do novo arquivo, não do comprovante persistido), `admin.comprovantes.tsx` (sempre `readOnly`, com `valorCadastrado` para mostrar divergência ao Analista/Gerência), `ConsolidadoCompetencia.tsx` (edição inline de envios ainda não decididos, a partir do Resumo da Competência).

### `ResumoPagamento`
- **Finalidade:** card de resumo do **documento atual** (não da competência inteira) antes de confirmar/persistir.
- **Props:** `arquivos: { nome, documentos }[]` (**atualizado — Etapa 3**; antes era `{ nome, tipos }[]`, e antes disso `arquivo: string` único), `beneficiarios: BeneficiarioPagamento[]`, `competencia: string`, `isRetroativo: boolean`, `justificativaAtraso?: string`, `gruposExtraidos: { beneficiarioId, campos }[]`.
- **Comportamento:** somente leitura, lista todos os arquivos anexados (nome + tipos, via `tiposDoArquivo()`) e todos os campos de todos os grupos em formato `<dl>`, valor bruto de cada campo (**Etapa 4** removeu o tratamento especial que existia para `tipoAssistencia` — não é mais um campo do formulário, ver seção 3.21).
...
- **(Ajuste pontual pré-commit da Etapa 7, revertido depois)** Campo "Modalidade/tipo do plano" do dependente, nos fluxos de Inclusão de Dependente e de Mudança de Plano (dependente migrando para plano diferente do titular), chegou a virar `<select>` estruturado (Individual/Familiar / Empresarial) reaproveitando o vocabulário `ModalidadePlano` do Módulo de Pagamento — **revertido a pedido do usuário** para o padrão original: campo de texto livre + toggle "Empresarial" separado com aviso de fatura técnica, igual ao Requerimento de Primeira Inclusão. Detalhes em `docs/PORTAL_SERVIDOR_NAVEGACAO_E_CADASTRO.md`, seção 6. Nunca herdado do titular, em ambas as versões.
- **(Novo — ajuste pontual pós-Etapa 7)** Elegibilidade avaliada por item financeiro, não pelo documento inteiro: um mesmo arquivo pode ter mensalidade (reembolsável) e odontológico/multa/taxa administrativa/juros (não reembolsável) ao mesmo tempo — só o(s) item(ns) não reembolsável(is) é(são) desconsiderado(s), sem bloquear o restante; bloqueio total só quando `valorElegivel = 0`; decomposição (valor total/elegível/não reembolsável) visível para Servidor, Analista, Gerência e no Resumo da Competência — seções 3.26, 7, 13 (decisão 38).

**Commitado no git** (branch `feature-modulo-pagamentos`, ver seção 13 para lista de commits): fluxo do Servidor (Etapa 1 original — nomenclatura antiga, anterior ao plano de 7 etapas do stakeholder), fluxo do Analista + sino + detalhe (Etapa 2 original), 2ª alçada da Gerência (Etapa 3 original — **posteriormente removida**, ver decisão 30), alertas de competências sem comprovante, os 3 novos estados do fluxo de envio + campo Pagador + alerta de competência incompleta (commit `cd324da`), multi-arquivo/consolidação/navegação do Resumo (commit `f15a028`), Etapa 1 do plano de alinhamento de stakeholder — retroativo com aprovação única (commit `5aeee9f`), Etapa 2 — seleção de beneficiários por grupo (commit `d03d8f7`), Etapa 3 — documentos obrigatórios + cobertura por beneficiário (commit `730f828`), Etapa 4 — taxonomia de campos + situações não reembolsáveis (commit `1dc2190`), Etapa 5 — justificativas obrigatórias (commit `7988d58`), Etapa 6 — operadora divergente (commit `de16ad0`), Etapa 7 + os 2 ajustes pontuais pré-commit (divergência cadastral × documental, seção 3.25; modalidade do dependente estruturada) juntos no commit `2d1316f`. Com esse commit, o plano completo de alinhamento de stakeholder (7 etapas) está implementado, testado e documentado.

**Ainda não commitado no momento da escrita desta atualização** (verificar `git status` para confirmar o estado exato): a elegibilidade por item financeiro (ajuste pontual pós-Etapa 7, seção 3.26) — aguardando autorização explícita do usuário para commit, conforme processo já estabelecido.
...
8. **(Atualizado — Etapa 4) Auto-confirmação em `ConferenciaBeneficiarios` raramente dispara** — o campo `banco` (que sempre vinha com confiança "nenhuma") foi removido na Etapa 4, mas `dataPagamento` continua sempre gerado com confiança "media" (`gerarCamposExtraidos`, `ocr-mock.ts`), então a regra "todos os campos em alta confiança" ainda quase nunca é satisfeita — o Servidor continua precisando clicar "Confirmar" manualmente na maioria dos envios.
...
10. ~~`justificativaDivergencia` no `Comprovante` — campo existe no tipo mas não é mais escrito ativamente por nenhum fluxo atual.~~ **Resolvida — Etapa 5 (seção 3.22):** generalizado para `justificativasDivergencia?: { beneficiarioId: string; texto: string }[]` e agora escrito ativamente pelo Servidor no envio, quando o valor diverge do cadastro.
...
18. **(Novo) Correção de bug crítico: tela em branco ("Cannot read properties of undefined (reading 'filter')") ao abrir um comprovante retroativo antigo (Analista/Gerência) ou ao visualizar o Resumo da Competência** — reportado pelo usuário testando a aplicação. Causa raiz: `Comprovante`s persistidos em `localStorage` **antes** da renomeação de `ArquivoAnexado.tipos` para `documentos` (Etapa 3, seção 3.20) ficaram com `arquivo.documentos === undefined`; toda a cadeia de itens financeiros (`getItensFinanceiros`, seção 3.26) e funções relacionadas (`tiposDoArquivo`, `getCoberturaDocumental`, `todosDocumentosComCoberturaDefinida`, `getDivergenciaBoletoComprovante`, `processarNovoArquivo`/`confirmarReenvio` em `reenvio-comprovante.ts`) acessavam `arquivo.documentos.filter(...)`/`.map(...)`/`.find(...)` sem checar se o campo existia, derrubando a página inteira (error boundary do TanStack Start) assim que qualquer um desses comprovantes antigos era lido — no `admin.comprovantes.tsx` ao clicar "Visualizar" num retroativo, e no `ConsolidadoCompetencia.tsx` (Resumo da Competência) para qualquer comprovante da competência sendo exibida. **Reproduzido de forma determinística** injetando no `localStorage` um `Comprovante` com `arquivos: [{ nome, tipos: [...] }]` (formato antigo) em vez de `{ nome, documentos: [...] }`. **Corrigido** adicionando `?? []` em todo ponto de leitura de `arquivo.documentos` nos 3 arquivos citados — comprovantes no formato antigo agora degradam graciosamente (não exibem tipos/itens para aquele arquivo específico) em vez de quebrar a tela inteira. Não foi feita migração de dados (não há necessidade de "corrigir" o registro antigo, só de não quebrar ao lê-lo).
...
38. **(Ajuste pontual pós-Etapa 7) Elegibilidade avaliada por item financeiro, não pelo documento inteiro — booleano manual "documento misto" rejeitado explicitamente** — a partir de um exemplo real (boleto Hapvida com mensalidade e odontológico como linhas separadas do mesmo documento), o usuário apontou que a regra da Etapa 4 (seção 3.21/33) — 1 situação não reembolsável bloqueia o beneficiário inteiro — estava incorreta para documentos que combinam itens reembolsáveis e não reembolsáveis: a IA deveria "liberar o reembolso do valor do plano, mas não da taxa administrativa que está sendo apontada no arquivo", mantendo o bloqueio total só quando não sobra nenhum valor elegível. Antes de implementar, propus como caminho mais simples um booleano manual `documentoMisto` — o usuário rejeitou explicitamente: *"não quero utilizar um booleano manual... a regra correta é que a elegibilidade seja avaliada por item/valor identificado no documento, e não pelo documento inteiro."* Optou-se então por um novo tipo `ItemFinanceiro[]` gerado pelo mock por documento (não um flag), com elegibilidade e decomposição (`valorTotal`/`valorElegivel`/`valorNaoReembolsavel`) sempre recalculadas por item — mesmo espírito de "regra de negócio real, não atalho de mock" já seguido em outras decisões (ex: decisão 33, taxonomia literal por tipo documental). Ao aprovar o modelo, o usuário corrigiu um ponto específico da proposta: **não** redefinir silenciosamente o significado do campo `valor` existente (que continuaria bruto/total, não elegível) — `valorTotal`, `valorElegivel` e `valorNaoReembolsavel` deveriam ser consultáveis separadamente, e a divergência cadastral deveria comparar `valorElegivel` (não o bruto) com `valorCadastrado`. Decisão de manter a convenção de nome de arquivo já validada (`_misto` como sufixo, preservando o comportamento 100%-bloqueio já testado quando ausente) em vez de criar um novo mecanismo de mock — ver seção 3.26.
39. **(Correção de bug + ajuste de regra) `arquivo.documentos` sem guarda derrubava a tela inteira em comprovantes antigos; status legados "aguardando" tratados incorretamente como somente-histórico** — o usuário reportou, com prints, dois erros: tela em branco ("Cannot read properties of undefined (reading 'filter')") ao abrir um retroativo antigo no Admin, e o mesmo erro no Resumo da Competência. Reproduzido de forma determinística injetando no `localStorage` um `Comprovante` no formato anterior à renomeação `tipos`→`documentos` (Etapa 3): `arquivo.documentos` ficava `undefined`, e várias funções (`getItensFinanceiros`, `tiposDoArquivo`, `getCoberturaDocumental`, `getDivergenciaBoletoComprovante`, `processarNovoArquivo`/`confirmarReenvio`) acessavam `.filter()`/`.map()`/`.find()` sem checar — corrigido com `?? []` em cada ponto de leitura (seção 12, item 18). Na mesma rodada, o usuário apontou que um registro retroativo seu, preso no status legado `retroativo_aguardando_gerencia`, estava incorretamente bloqueado como "somente consulta" — segundo a regra de aprovação única (decisão 30), esse status é **a mesma pendência** que `retroativo_aguardando_aprovacao`, só com o nome anterior à Etapa 1, então deveria continuar acionável por qualquer um dos dois papéis, não congelado como histórico. Corrigido: `retroativo_aguardando_analista`/`retroativo_aguardando_gerencia` entraram em `statusComAcaoDisponivel`, `proximoStatusAprovacao()` e a lógica de recusa (`statusRecusa`) — resolvem exatamente como `retroativo_aguardando_aprovacao`; o rótulo dos 2 status foi unificado para "Retroativo — Aguardando Aprovação" (sem "(legado)"), e a mensagem "Registro legado (2ª alçada) — somente consulta" foi removida. **`retroativo_devolvido` foi deliberadamente excluído** dessa mudança e continua somente leitura — representa uma decisão já tomada (Gerência devolveu ao Analista sob a antiga 2ª alçada), não uma pendência equivalente à aprovação única; o próprio usuário reafirmou esse status como somente-consulta ao descrever a regra.
...
Etapas 1 (retroativo com aprovação única), 2 (seleção de beneficiários por grupo), 3 (documentos obrigatórios + cobertura por beneficiário), 4 (taxonomia de campos + situações não reembolsáveis), 5 (justificativas obrigatórias), 6 (operadora divergente do cadastro) e 7 (solicitar requerimento + indicador de prazo) estão implementadas, testadas e commitadas (junto dos 2 ajustes pontuais pré-commit, commit `2d1316f`). **O plano completo de 7 etapas do alinhamento de stakeholder está finalizado** — nenhuma etapa nova está aprovada além destas; próximos incrementos exigiriam um novo ciclo de alinhamento com o stakeholder. Um ajuste pontual adicional, pós-Etapa 7, foi implementado e testado depois disso: elegibilidade por item financeiro (seção 3.26, decisão 38) — aguardando autorização de commit.
