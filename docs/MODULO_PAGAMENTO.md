...
- **Props:** `titulo?: string`, `campos: CampoExtraido[]`, `onChange?: (campos) => void`, `valorCadastrado?: number` (ativa divergência de `valor`), `nomeTitular?: string` (ativa divergência de `pagador`), `situacaoNaoReembolsavel?: SituacaoNaoReembolsavel` (**novo — Etapa 4**, banner de alerta), `divergenciaBoletoComprovante?: boolean` (**novo — Etapa 4**, badge no campo `valor`), `readOnly?: boolean` (default `false`).
- **Comportamento:** banner vermelho no topo do form quando `situacaoNaoReembolsavel` é informado ("Não elegível — {situação}. ... bloqueada até revisão da GERDAB") — substituiu o antigo badge por campo (`tipoAssistencia`), já que essa chave não existe mais. Por campo, mostra label + input; badges condicionais "Não identificado" (campo vazio), "Divergente" (valor ou pagador fora do esperado), "Valor diverge do boleto anexado" (**novo — Etapa 4**, só no campo `valor` quando `divergenciaBoletoComprovante`), "Preenchido manualmente" (`origem === 'manual'`) ou indicador de confiança (ícone + texto); linha adicional "Origem: {arquivoOrigem}" quando o campo tem essa informação. Editar sempre seta `origem: 'manual'`.
